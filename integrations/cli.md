...
通过 `audison audit` 命令直接审查 AI 生成代码，零代码集成，适合本地开发、CI/CD 流水线和 pre-commit hook。
...
```bash
audison audit auth.py -r "检查 SQL 注入、密码哈希和认证漏洞"
```
...
```bash
cat auth.py | audison audit - -r "检查安全漏洞" --json
```
...
```bash
audison audit auth.py -r "检查 SQL 注入和认证漏洞" --markdown
```
...
1. AI 生成代码 → 保存为 `.py` 文件
2. 运行 `audison audit <file> -r "<审查需求>"`
